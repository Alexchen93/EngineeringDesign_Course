...
<body>
  ...
  <script>
    ...
    function updateStatistics() {
      // 更新統計資訊
      document.getElementById('totalClothes').textContent = clothesData.length;
      
      // 計算今日使用次數
      const today = new Date().toISOString().split('T')[0];
      const todayUsage = historyData.filter(h => h.date.startsWith(today)).length;
      document.getElementById('todayUsage').textContent = todayUsage;

      // 從天氣資訊中獲取當前溫度範圍並計算平均值
      fetch('/recommend')
        .then(response => response.json())  // 改為 JSON 格式
        .then(data => {
      ...
      const mostUsed = Object.entries(positionCounts)
        .sort((a, b) => b[1] - a[1])[0];
      if (mostUsed) {
    ...
    async function getCards() {
      try {
        ...
        if (viewMode === 'list') {
          renderListView(container, clothesData);
        } else {
          renderGridView(container, clothesData);
        }
      } catch (error) {
    ...
    function renderClothes() {
      ...
      if (viewMode === 'list') {
        renderListView(contentView, clothesData);
      } else {
        renderGridView(contentView, clothesData);
      }
    }
    ...
    function renderListView(container, items) {
      ...
      filteredItems.forEach(item => {
        html += `
          <tr>
            ...
            <td>
              <button onclick="editItem('${item.uid}')" class="btn-action edit">編輯</button>
              <button onclick="deleteItem('${item.uid}')" class="btn-action delete">刪除</button>
            </td>
          </tr>
        `;
      });
      ...
      style.textContent = `
        ...
        .data-table tr:hover {
          background-color: rgba(27, 38, 79, 0.05);
        }
    ...
    function renderGridView(container, items) {
          ...
          ${filteredItems.map(item => `
            <div class="grid-item">
              <div class="grid-item-header">
                <h3>${item.cloth || '未命名衣物'}</h3>
                <div class="grid-item-actions">
                  <button onclick="editItem('${item.uid}')" class="btn-action edit">編輯</button>
                  <button onclick="deleteItem('${item.uid}')" class="btn-action delete">刪除</button>
                </div>
              </div>
              <table class="grid-item-details">
                <tr>
                  <th>款式：</th>
                  <td>${item.model || '-'}</td>
                </tr>
      ...
      style.textContent = `
        ...
        .grid-item-details th,
        .grid-item-details td {
          padding: 12px 15px;
          text-align: left;
          border-bottom: 1px solid var(--border);
        }
        ...
        .grid-item-details tr:last-child th,
        .grid-item-details tr:last-child td {
          border-bottom: none;
        }
      `;
      container.appendChild(style);
    ...
    function updateFilterOptions(data) {
      ...
      for (let i = 1; i < data.length; i++) {
        const row = data[i];
        if (row[6]) tops.add(row[6]);
        if (row[7]) topColors.add(row[7]);
        if (row[8]) topStyles.add(row[8]);
        if (row[9]) bottoms.add(row[9]);
      }
    ...
    function filterHistory() {
      ...
      fetch('/similarHistory')
        .then(response => response.json())
        .then(data => {
          // 篩選符合條件的資料
          const filteredData = data.filter((row, index) => {
            if (index === 0) return false; // 跳過標題行
            return (!top || row[6] === top) &&
                   (!topColor || row[7] === topColor) &&
                   (!topStyle || row[8] === topStyle) &&
                   (!bottom || row[9] === bottom);
          });
